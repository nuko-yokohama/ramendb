...
WITH t AS
(
  SELECT tl.tid, tl.name, round(ST_Distance((SELECT geography FROM shops_location_mv WHERE sid = $1), tlmv.geography)::real) as dist FROM target_location_mv tlmv JOIN target_location tl ON (tlmv.tid = tl.tid)
)
